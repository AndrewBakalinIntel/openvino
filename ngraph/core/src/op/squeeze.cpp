...
namespace squeeze
{
    ...
    bool evaluate(const HostTensorPtr& arg0, const HostTensorPtr& arg1, const HostTensorPtr& out)
    {
        ...
        auto axes = normalize_axes("", std::vector<int64_t>(arg1->get_data_ptr<ET>(), arg1->get_data_ptr<ET>() + axes_num), data_rank);
        ...
        runtime::reference::copy(
            arg0->get_data_ptr<char>(), out->get_data_ptr<char>(), shape_size(out->get_shape()) * out->get_element_type().size());
        return true;
    }
