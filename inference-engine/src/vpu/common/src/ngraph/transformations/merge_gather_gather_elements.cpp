...
bool MergeGatherGatherElements::run_on_function(std::shared_ptr<ngraph::Function> f) {
    ...
    const auto m = std::make_shared<ngraph::pattern::Matcher>(transpose, "GatherReshapeTransposeMatcher");
    for (const auto& node : f->get_ordered_ops()) {
        if (m->match(node)) {
            ...
            const auto gatherDataShape = std::make_shared<ngraph::opset6::ShapeOf>(m_gatherData);
            const auto gatherIndicesShape = std::make_shared<ngraph::opset6::ShapeOf>(m_gatherIndices);
